# calculate_amplitude_correlations.R
# Script to load data for 5 different perturbation amplitudes (0.0, 0.25, 0.50, 0.75, 1.0),
# compute their perturbation-response geometry distance matrices, calculate Pearson correlations
# between them under different observation windows, and generate corresponding heatmaps.
...
log_info("Starting aligned multi-window 5-amplitude correlation analysis...")

# 1. Define amplitudes and their raw data directories
amplitudes <- c(0.0, 0.25, 0.50, 0.75, 1.0)
dirs <- c(
    "0.0"  = "data/raw_amp0.0",
    "0.25" = "data/raw_amp0.25",
    "0.50" = "data/raw_amp0.5",
    "0.75" = "data/raw_amp0.75",
    "1.0"  = "data/raw"
)
...
compute_correlation_matrix <- function(max_t) {
    dist_matrices <- list()
    
    for (amp_name in names(dirs)) {
        ds <- datasets[[amp_name]]
        
        # Subset to the requested observation window
...
log_info("Computing correlation matrix for Window = 200 (t <= 4200)...")
cor_matrix_200 <- compute_correlation_matrix(4200)

log_info("Computing correlation matrix for Window = 50 (t <= 4050)...")
cor_matrix_50 <- compute_correlation_matrix(4050)

# 3. Save matrices to CSV
save_matrix_csv(cor_matrix_200, "outputs/results/amplitude_correlation_matrix_win200.csv")
save_matrix_csv(cor_matrix_200, "outputs/results/amplitude_correlation_matrix.csv") # default
save_matrix_csv(cor_matrix_50, "outputs/results/amplitude_correlation_matrix_win50.csv")

log_info("Correlation Matrix - Window = 200:")
print(cor_matrix_200)

log_info("Correlation Matrix - Window = 50:")
print(cor_matrix_50)
...
plot_heatmap <- function(cor_matrix, file_path, title_text) {
    ...
    tryCatch({
        ...
        p <- ggplot(melted_cor, aes(x = Var1, y = Var2, fill = Correlation)) +
            ...
            geom_text(aes(label = sprintf("%.4f", Correlation)), color = "white", size = 4, fontface = "bold") +
        ...
        log_info("ggplot2/reshape2 failed, falling back to base R...")
    })
    ...
    if (!plot_success) {
        ...
        for (i in 1:n_amps) {
            for (j in 1:n_amps) {
                ...
                text(i, n_amps - j + 1, sprintf("%.4f", val), col = text_col, font = 2, cex = 1.0)
            }
        }
        par(op)
        dev.off()
...
# 5. Plot heatmaps
plot_heatmap(cor_matrix_200, "outputs/figures/amplitude_correlation_heatmap.png", "Correlation between perturbation of different intensities\n(Window = 200 generations)")
plot_heatmap(cor_matrix_200, "outputs/figures/amplitude_correlation_heatmap_win200.png", "Correlation between perturbation of different intensities\n(Window = 200 generations)")
plot_heatmap(cor_matrix_50, "outputs/figures/amplitude_correlation_heatmap_win50.png", "Correlation between perturbation of different intensities\n(Window = 50 generations)")

log_info("All heatmaps generated successfully!")
